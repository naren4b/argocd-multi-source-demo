...
def getGitCommand(sources):
    for repo in sources:
        ...
        if os.path.exists(format(os.path.join(current_directory, ref_dir))):
            executeMyCommand("cd  {}".format(os.path.join(current_directory, ref_dir)))
            executeMyCommand("git pull origin " + targetRevision)
        else:
            executeMyCommand(
                GIT_CLONE_BASE_COMMAND + " " + targetRevision + " " + repoURL
            )
